...
**What's built this milestone (Sessions 42–56):**
- **Queries screen — complete** (`/study/[studyId]/queries`). The study-wide query worklist (CRA/DM workstation): **Queries** and **Edit Checks** tabs, each its own table; rows open a 420px slide-in thread panel (reuses `subject-record.css`); the Subject-ID link deep-links into the record (`?form=&field=`). Role-gated actions (CRC respond · CRA resolve+respond · DM manage dropdown · PI respond · Sponsor/Admin read-only, Sponsor sees no Edit-Checks tab). A **live, role-aware sidenav badge** counts the queries needing the active role's action and updates on role switch / resolution. CA-0801 seeded to ~15 query items for a demonstrative workstation.
- **App-wide table-sort standard** — one shared `useTableSort` hook + `<SortTh>` header component (asc → desc → clear, one column at a time, arrow hidden until hover) wired across **every list table** (Animals, Queries, Edit Checks, Studies, Sites, Data-Entry drill-down, Barn record). **ID/code columns never wrap** (`white-space: nowrap` on every identifier cell).
...
**Next priority: the portfolio site at elisatron.com** — the case-study site that frames Arken EDC (and Elisa's other work) for hiring managers; pull the narrative from `PORTFOLIO_GUIDE.md` + `CASE_STUDY.md`. *(Secondary, still open: BR-2502 dashboard wiring — CA-0801 dashboards are already on live aggregates; do the same for BR-2502.)*
...
> **Data model (important):** Supabase is the **read-only seed source**. `app/lib/session-store/` (`useStudySession` / `StudySessionProvider`) hydrates the dataset once per tab into `sessionStorage`; every screen reads from it; `update()` mutates in session and **never writes back to Supabase**. Only `hydrate.ts` reads Supabase. Session-store key is at **v23** (`arken_session_store_v23`; bump on dataset-shape changes).
>
