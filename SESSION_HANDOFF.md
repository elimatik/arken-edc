...
Last updated: 2026-06-17 | Session 48 COMPLETE — **Two quick fixes:** (1) the login + access-agreement (NDA) credential fields are now wrapped in a `<form autoComplete="off" data-form-type="other" onSubmit={preventDefault}>` (LastPass keys off the `<form>`, so `data-form-type="other"` + per-input `data-lpignore`/`data-1p-ignore` together suppress the autofill overlay on the demo credential); (2) the batch grid **Status column is simplified to one chip** — a red **"Error"** chip when any cell in the row has an open edit check (priority), a neutral **"Saved"** chip when the row has data and no error, and **nothing** when empty (the old ○ Empty / ✎ Changed / ✓ Saved / ⚠ Alert variants and the EC- count chip are gone). UI-only (session key still **v23**). Prior — Session 47 — **Three small fixes:** (1) login + NDA inputs disable browser autofill (`autocomplete="off"`, password `autocomplete="new-password"`, non-standard `name`s + `data-lpignore`/`data-1p-ignore`); (2) the batch grid's shared **Visit-date** label now sits **above** the input in the design-system field-label style (uppercase, `--text-xs`, `--weight-medium`, caps tracking, `--color-text-tertiary`); (3) the batch grid's **EC- indicator is a non-clickable `<span>`** (no panel) with the tooltip "Review this value on the individual subject record". UI-only (session key still **v23**). Prior — Session 46 — **Five fixes: batch picker/withdrawn filtering, the stale-query-badge root cause, and edit-check amber styling.** The big one: **Supabase silently capped hydration at 1000 rows**, truncating `field_values` (≈1479 across studies) — which orphaned the queries / edit checks pointing at the dropped values, so a sidebar badge counted a query the form couldn't show. `hydrate.ts` now **pages every table** (1000-row chunks) so all rows load; the query badge is additionally hardened to only count queries that resolve to a field value on the instance. The batch picker drops forms every non-withdrawn animal has already completed and **excludes withdrawn animals** (rows + due counts). Edit checks now turn the **input itself amber** (amber-200 border + amber-50 bg) on SubjectRecord / scoped forms / batch grid, fire **on blur** (not per keystroke), and the inline text reads "[EC-…] Value outside expected range (Normal: X–Y unit)". Session key **v23** (forces a fresh full hydrate). Prior — Session 45 — **Batch grid simplified to data-entry + change-reason only.** Removed the Remarks dropdown, Queries mode, and all flag icons from the batch grid — **queries and SDV happen on the individual subject record only** (the orange **EC- indicator** stays as a non-interactive validation flag). The form picker now **de-dupes** (suggested forms, then the *remaining* batch-eligible forms) and **opens on click** (no "Open entry grid" button). Added a shared **"Visit date" batch-header field** above the rows that auto-fills every animal without a saved visit date (a generic "batch header field" mechanism). Pending-Δ cells keep a default border (the dashed-red is on the Δ button only), and the Yes/No toggle is compact (fixed 48px buttons, left-aligned, not stretched). UI-only (session key still **v22**). Prior — Session 44 — **Batch grid gains the full Subject-Record per-cell flow.** Each cell now **live-persists** to that animal's own form instance, so the exact Subject-Record machinery works inline: a **per-cell dashed-red Δ button** opens the 380px change-reason panel (old→new, reason textarea, history; the bottom rfc-bar is now a summary shortcut "X fields have pending changes"); the **EC- icon → Edit Check panel** (explain → convert-to-query) and, in **Queries mode** (new Remarks dropdown — Off / Queries; SDV stays on the subject record), a **flag icon → Query panel** (raise/respond/resolve) — Scenario A (EC + separate flag) and Scenario B (converted EC folds into the unified query thread, EC icon gone) match the Subject Record. Colour consistency: amber for edit-check/query, dashed-red for pending Δ, **no green fill** on saved cells (green reserved for DM-approved Δ), blue focus ring. UI-only (session key still **v22**). Prior — Session 43 — **Batch Entry rebuilt faithful to `20-batch-entry.html` + BR-2502 finalized.** The batch grid now has **no checkboxes** (every visible animal is included), a **reason-for-change bar** (rfc-bar) that appears when re-editing previously-saved data (one reason for all modified cells, with Override-per-row) writing **change-reason deltas**, and **mirrors every row to that animal's own form instance** — the Subject Record shows the batch-entered data (verified: a value typed in the grid appears on the individual Vital Signs form with its edit check). Queries/SDV/form-flow CTAs stay only on the Subject Record. Two full-page views (`BatchPicker` form-cards → `BatchEntryGrid`). BR-2502 Day-0 Clinical Response is simpler (no "Response vs baseline"); Group G forms repeat; **Re-treatment** shows only when the flag is Yes and **Necropsy** only when the animal died. Session key **v22**. Prior — Session 42 — **BR-2502 restructured into 8 groups (one group per visit day) + the Batch Entry feature.** A `forms.batch_eligible` flag (the 10 BR Vital Signs + Clinical Response visit forms) drives a "Batch entry" button on the BR Animals list and the animal-level drill-down (hidden for PH/CA). Step 1 = a smart suggestion modal (forms due today, from each animal's enrollment date + visit windows D0±0/D3±1/D7±2/D14±3/D28±4); Step 2 = a full-screen grid (one row per animal, one column per field, inline editing, per-cell edit checks that flag but never block, Save-all → summary toast). Veterinary pen/herd batch entry — no human-EDC equivalent. **137 forms / 989 fields**, session key **v21**.
