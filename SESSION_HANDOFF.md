...
Last updated: 2026-06-14 | Session 36 COMPLETE — **PH-2401 replaces CX-2501** (Phytogenic Feed Additive Broiler Growth Performance Trial — 5 groups / 10 pen forms, 2 arms, single house, 42 days). NEW architecture: **barn/house-scoped forms** (`forms.scope`, `form_instances.barn_id`) + a **Barn Record page** rendering the house-level **Daily Environmental Log** with live ammonia/temp edit checks — excluded from the pen sidebar. NEXT: deferred form polish (SDV panel, sidebar deep-link, query re-open, conditional demographics).
