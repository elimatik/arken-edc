...
Last updated: 2026-06-14 | Session 38 COMPLETE — **recurring forms → individual per-visit forms**: PH-2401 Body Weight & Flock Health each become 6 day-forms (D7–D42), BR-2502 Vital Signs (5: D0–D28) + Clinical Response (4: D3–D28), each its own sidebar item with independent status/query/SDV. Removed the Assessment-day selector (now a read-only "Day N" display from the form name); the Weekly summary rows carry status icons and jump to the matching visit form. NEXT: deferred form polish (SDV panel, sidebar deep-link, query re-open, conditional demographics).
