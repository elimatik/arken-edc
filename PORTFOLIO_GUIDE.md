...
- **CX-2501** — *Anticoccidial Efficacy in Broiler Chickens* — poultry, group-housed (Site → **Room → Pen** — pen-level capture, no per-bird demographics; the housing label relabels to *House/Room* for chicken). Randomized complete block, **4 arms** (unchallenged/untreated · challenged/untreated · challenged/test · challenged/reference), 8 pen-level forms.
- **BR-2502** — *Bovine Respiratory Disease Treatment Trial* — cattle, individual records (Site → Barn → Pen → Animal; animals share a pen but each carries its own record). 3 arms, 4 feedlots, rolling enrollment, 8 animal forms, and **age-class heart-rate validation** (calf ≤6 mo 100–140 vs adult 48–84 bpm).
...
- **`app/supabase/generate-seed.mjs`** — generates `seed.sql` (69 forms / 401 fields) from each study's own form tree + field definitions, hierarchy (multi-site), subjects, and demo data. Edit it, re-run `node generate-seed.mjs`, then `db reset`.
