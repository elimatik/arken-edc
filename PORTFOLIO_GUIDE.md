...
- **CX-2501** — *Anticoccidial Efficacy in Broiler Chickens* — poultry, group-housed (Site → **Room → Pen** — the **pen IS the subject**: the hierarchy ends at Pen and clicking a pen opens its Subject Record, no per-bird demographics; the housing label relabels to *House/Room* for chicken). Randomized complete block, **4 arms** (unchallenged/untreated · challenged/untreated · challenged/test · challenged/reference), **14 pen-level forms in 6 groups**.
...
- **`app/supabase/generate-seed.mjs`** — generates `seed.sql` (94 forms / 510 fields) from each study's own form tree + field definitions, hierarchy (multi-site), subjects, and demo data. Forms nest into `grp()`/`leaf()` groups; `sec()` tags fields with in-form sections. Edit it, re-run `node generate-seed.mjs`, then `db reset`.
