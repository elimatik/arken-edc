...
| `/study/[studyId]/queries` | Queries screen (study-wide query + edit-check worklist; row → slide-in thread panel) |
...
**Queries screen** (`/study/[studyId]/queries`, the CRA/DM workstation) — the per-record query thread already lived on the Subject Record; this is the **study-wide roll-up** of every query + open edit-check across all subjects/forms. A **stat strip** (Total open / Raised / Responded / Resolved / Avg days open) over a **filter toolbar** (search by query-ID / subject / field · status · form · site · "needs my action" · sort) and a dense table: Query ID (`Q-###` / `EC-###`), Subject (linked), Site, Form, Field, a **status chip** (Raised orange / Responded amber / Resolved green / Edit-check orange), opened date, **days-open** (red past the SLA — >7 raised, >3 responded), last action, and **role-gated actions**. A **row click opens an inline 420px slide-in thread panel** (the same thread as the record, reusing `subject-record.css`); the **Subject-ID link instead navigates** to the record and **deep-links straight to that field's panel** (`?form=&field=`). Role-gating mirrors the record: CRC responds, CRA resolves + responds, DM gets a full manage dropdown (incl. close-without-response), PI responds, **Sponsor / Admin are read-only — and Sponsor never sees `EC-` rows** (blinding). Edit-checks are surfaced but **not actionable here** (resolved on the record), so the query log and the anomaly list stay one screen for the data manager.
