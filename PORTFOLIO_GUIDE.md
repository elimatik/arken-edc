...
**Built:** login (+ access agreement), study selector, role-aware app shell, role dashboards (**CA-0801 has bespoke CRC/PI/DM dashboards wired to live session-store aggregates** — a stacked enrollment graph with a dashed target marker, plus compliance / safety / data-quality and a compact open-queries list, all derived from real subjects/forms/queries), Data Entry drill-down (**role-gated Add Site / Barn / Pen / Subject** — Admin adds sites, CRC/DM add housing, CRC/CRA add animals — all into the session store; Admin can browse the hierarchy but not open individual records — *"Data entry is restricted to clinical roles."* **Note:** full site management (staff, activation, configuration) belongs in **Settings** — the Add Site button in the drill-down is a quick shortcut only), **Animals list**, Subject Record (clickable full-path breadcrumb · **subject switcher** · in-form section headers · completed/withdrawn read-only with a preservation banner · **repeating-table forms** for ConMed / Adverse Events / Protocol Deviations — add/edit/delete entries via a slide-in panel, with auto AE numbers + VeDDRA look-up), the session-store data layer, and the **grouped form layer** — three clinically-realistic studies (**PH-2401** broiler / **HF-3001** heifer / **CA-0801** canine multi-site), each with its **own** form tree (95 / 121 / **221** fields; **109 forms total**), forms nested into collapsible visit/info **groups** (`parent_form_id`), a dedicated **Randomization** form linking each arm to a treatment lot/batch/kit (the inventory bridge — Case Study 4), and **species-specific validation** (`species_ranges`, 6 species incl. chicken/ammonia) that auto-raises and auto-resolves inline edit-check queries.
