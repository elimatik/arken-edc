...
- **`app/supabase/generate-seed.mjs`** — generates `seed.sql` (90 forms / 528 fields) from each study's own form tree + field definitions, hierarchy (multi-site), subjects, and demo data. Forms nest into `grp()`/`leaf()` groups; `sec()` tags fields with in-form sections; `study.barnForms` declares **house-scoped** forms (rendered on the Barn Record). Edit it, re-run `node generate-seed.mjs`, then `db reset`.
