...
- **CA-0801** — *DermAlliv™ Canine Atopic Dermatitis Study* (Protocol DERM-2026-104) — companion, individual records (Site → Subject + owner). The richest study: **3 sites** (Austin / Denver / Raleigh), 13 dogs (incl. a screen failure), a 53-form eCRF (Screening → Randomization → 4 follow-up visits → End of Study + AE / Protocol Deviation / Withdrawal Form / ConMed / a read-only **ePRO** owner-diary stub), and **wired CRC / PI / DM dashboards** (enrollment, compliance, safety, data-quality aggregates). The demo data is **internally consistent**: the three *Completed* dogs carry fully-populated visit forms with a realistic CADESI-04 / PVAS improvement trajectory across all visits and every query resolved (no finalized-but-empty forms, no open queries on a completed subject); the *Withdrawn* dog has real values up to her withdrawal and "not started" visits after; *Active* dogs carry the open/responded queries.
...
- The dataset lives in **`sessionStorage`** (key **`arken_session_store_v34`** — bumped on any dataset-shape change) behind a React context — every screen reads it through **`useStudySession()`**, never calling Supabase directly.
...
| `/study/[studyId]/sites/[siteId]` | Site Record — two tabs (`?tab=overview\|forms`): Overview config + site-scoped form flow; Admin edits, clinical roles read-only |
| `/study/[studyId]/barns/[barnId]` | Barn / House Record — two tabs: Overview (House info / Biosecurity / Equipment-calibration alerts + pen summary) + barn-scoped form flow (Daily Environmental Log, Feed Delivery, House Close-out) |
| `/study/[studyId]/data-entry/[subjectId]` | Subject Record (`?form=&field=` deep-links to a form + opens that field's query/EC panel) |
| `/study/[studyId]/data-entry/[subjectId]/[formId]` | Subject Record, form pre-selected |
| `/study/[studyId]/batch-entry` | Batch Entry grid (BR-2502; `?loc=` + `?from=` params; one row per animal, shared visit date, per-cell mirror to each animal's instance) |
| `/study/[studyId]/queries` | Queries screen (study-wide worklist; Queries / Edit-Checks tabs, live role-aware sidenav badge, row → slide-in thread panel) |
| `/study/[studyId]/visits` | Visits scheduler (Upcoming & Overdue / Visit History tabs; target-date + window derivation, urgency stat strip, sortable; Sponsor sees aggregate only) |
| `/study/[studyId]/audit-trail` | Audit Trail (21 CFR Part 11 event log, derived live from the session store; sortable, role-scoped, row → slide-in detail; CRA / DM / PI / Admin only) |
...
- **`app/lib/useTableSort.ts` + `app/components/common/SortTh.tsx`** — the **shared table-sort standard** used by every list table (Animals, Queries, Studies, Sites, Data-Entry drill-down, Barn record): click a header to cycle **ascending → descending → clear**, one column at a time, with the arrow icon hidden until hover. Generic styling lives in `globals.css` (`.col-sortable`). Identifier columns carry `white-space: nowrap` so IDs never break across lines.
...
**Queries screen** (`/study/[studyId]/queries`, the CRA/DM workstation) — the per-record query thread already lived on the Subject Record; this is the **study-wide roll-up** of every query + open edit-check across all subjects/forms, built faithfully from the `15-queries-list.html` prototype. It splits the worklist into **two tabs** — **Queries** (`Q-###` manual queries, the Raised → Responded → Resolved flow) and **Edit Checks** (`EC-###` open auto edit-checks) — each with a count badge and **its own table**; switching tabs filters in place. The **Queries table** carries: Query ID, Subject (linked), Site, **Field** (the field label with an **inline code chip**, over a **group → form path** subtext — e.g. "Screening — Physical Examination"), **Value** (the field value related to the query), **Query text** (truncated, with the full text in the slide-in panel), a **status chip** (Raised orange / Responded amber / Resolved green), **days-open** (red past the SLA — >7 raised, >3 responded), **Assigned to** (derived from status — raised → CRC to respond, responded → CRA to resolve), and **role-gated actions**. Subject · Site · Status · Days-open · Assigned-to are **sortable columns** (the same click-to-cycle asc → desc → clear pattern as the Animals list), overriding the toolbar's default oldest-first. The **Edit-Checks table** is purpose-built for triage: Edit-check ID, Subject, Site, Field (+ form subtext), **Value** (the out-of-range number), **Normal range** (e.g. "38.3–39.2 °C", resolved from the species-range engine), an orange **"Edit Check"** status chip, and days-open — with no action column, because converting an edit-check is a clinical call that belongs next to the source (the slide-in panel carries the "resolve on subject record" note). A **row click opens an inline 420px slide-in thread panel** (the same thread as the record, reusing `subject-record.css`; the EC panel adds Value / Normal / Fired detail rows); the **Subject-ID link instead navigates** to the record and **deep-links straight to that field's panel** (`?form=&field=`). Role-gating mirrors the record: CRC responds, CRA resolves + responds, DM gets a full manage dropdown (incl. close-without-response), PI responds, **Sponsor / Admin are read-only — and Sponsor never sees the Edit-Checks tab** (blinding). The **sidenav Queries badge is live and role-aware** — it counts only the queries that need the active role's action (CRC/PI = raised; CRA/DM = raised + responded; Sponsor/Admin = none) and **updates reactively** when the role switches or a query is resolved.

**Audit Trail** (`/study/[studyId]/audit-trail`, the **21 CFR Part 11** event log) — a study-wide, read-only, immutable record of every action, **derived live from the session store** rather than a separate audit table: each saved field value is a *Data Entry* event, change-reason deltas become *Change Reason* (old → new + reason), the query thread becomes *Query Raised / Responded / Resolved*, plus *Edit Check*, *SDV Verified*, *Form Submitted / Locked*, and *Subject Enrolled / Withdrawn* — so the trail can never drift from the data it describes. Nine **sortable** columns (Timestamp · User + role · Action chip · Subject · Form · Field + code · Old value · New value · Details) on the shared sort standard, newest-first by default, with a full filter toolbar (search · action type · user · subject · form · date range) and a CSV-export stub; a row opens a 420px **slide-in detail panel**. The page wears its compliance identity — a slate **"21 CFR Part 11"** badge by the title and a banner noting the demo is session-based while production entries are permanent and tamper-evident. **Role-scoped:** CRA / DM / Admin see everything (Admin alone sees login events), **PI sees only subject-level events**, and **CRC / Sponsor have no access**. The trail captures **Randomization, Consent Obtained, Protocol Deviation, and Emergency Unblinding** milestones alongside data-entry/query/SDV events, and renders every timestamp in **UTC**.

**Visits** (`/study/[studyId]/visits`, the study-wide visit scheduler) — like the audit trail, it stores nothing of its own: a *visit* is **derived** as (subject × scheduled visit form), the schedule and target dates falling out of the form structure. Each study's visit days and **windows** are encoded (CA-0801: Day 14 ±2, 28 ±3, 56/84 ±5; BR-2502: Day 0/3/7/14/28; PH-2401: weekly to Day 42), the **target date** = the subject's baseline (enrolment / placement / randomisation) + the day offset, and a visit is *done* once its form reaches review. Two tabs: **Upcoming & Overdue** ranks every active subject's pending visits by urgency — **Overdue** (red, past target + window), **Due today** (amber), **Due this week** (blue), **Upcoming** (grey), with a signed days-to-target column (red when overdue); **Visit History** shows completed visits with the **variance** between the recorded and target dates (On time / Early / Outside window). A stat strip totals overdue / due-this-week / completed-on-time / outside-window / scheduled. Sorting is via column headers (including a Status sort that ranks by urgency), and the **Open** action deep-links straight to that visit's form, highlighted in the subject record. **Sponsor** sees the stat strip only (no subject-level table) — the rest get the full sortable tables with site filtering.

**Clinical-safety details** — the form layer carries real veterinary protocol logic, not placeholders. **Score legends** ride on the fields that need them: a persistent hint under each BR-2502 DART *Clinical Illness Score* (0 = Normal … 3 = Severe/recumbent), each CA-0801 **PVAS** (0 = no itching … 10 = extremely severe, owner-reported), and each **CADESI-04** score (the calculated 0–180 dermatitis index). CADESI also carries a **soft edit-check** — a value >120 is unusual, so it raises a non-blocking "verify with the investigator" EC- (the score is still accepted) — and a **"CADESI-04 worksheet" file-upload** sits right after each score for the source scoring sheet. The standout is a **food-safety HARD block**: a BR-2502 animal still inside its drug-withdrawal period **cannot be marked "Shipped"** on the End-of-Study form — submission is blocked with a red banner ("…not cleared for slaughter") that the CRC cannot override (only a DM, by recording a protocol deviation). And for the double-blind CA-0801 study, a DM/Admin-only **Emergency Unblinding** button reveals a subject's treatment arm behind a reason-required confirmation, flipping the header chip from **Blinded** to the actual arm and writing a permanent audit entry.

The same form layer also carries **GCP and traceability detail**. Every visit form opens with a read-only **Protocol version** field (AUTO badge) so the data is always traceable to the protocol in effect (CA-0801 = v2.1, etc.). Fields can be **conditional** (`showIf`): PH-2401's investigational feed additive carries a **second-observer counter-signature** that appears only for T02 (phytogenic) pens — required when shown, with the verifier name and timestamp auto-stamped once confirmed — because GCP requires independent verification of IP administration, and a hidden conditional field never blocks an unaffected pen's submission. The Subject Record adds a collapsible **Timeline** (enrolment → randomisation → each dated visit → AEs → queries → withdrawal/completion, click to jump to a form), and the CA-0801 dashboards gain a **Visit Windows** compliance card (on-time / outside-window / overdue, derived from each visit's recorded date vs its protocol target).

**Reports module** (`/study/[studyId]/reports`) — a two-column catalog + report view, **role-scoped** (CRC, a data-entry role, is excluded) and **blinding-aware**. **Eight clinical reports**: Study Status Summary, Enrollment & Disposition, Site Performance, Visit Compliance, Data Completeness, Query & Edit Check, **AE / SAE Roster**, **Concomitant Medications (ConMed) Log**, and SDV Completion — every figure derived live from the session store (no parallel tables; new panel entries flow straight into the reports). It carries the clinical-domain depth a real sponsor's team would look for:
- **Blinding-aware arm display** — one `shouldHideArms()` helper (`lib/study-config.ts`) called from **both** the reports and the dashboard's enrollment bar: a CRC/CRA/Sponsor on the double-blind CA-0801 sees a single neutral enrolment bar and no arm column; a DM/PI sees the full Group A/B split. Open-label studies (BR/PH) always show arms.
- **Regulatory citations in context** — a **REFLECT** statement banner (the veterinary CONSORT) on Enrollment & Disposition; an **IACUC approval + VICH GL6/GL9** ethics block on Study Status.
- **SAE reporting timelines** (ICH E6 R2 §5.17 / VICH GL42) — PI-aware date, sponsor-notified date, **days-to-notify** (green ≤1, red >1), **report-due** (+7 d fatal/life-threatening, +15 d other), and a **filed-on-time** chip that flips to red **"Overdue"** automatically once the due date passes (PH-2401's seeded SAE is past due and shows red).
- **Washout compliance flag** (CA-0801) — `stop_date + protocol washout days` vs the enrolment date; an ongoing/insufficiently-washed immunosuppressant flags an amber "⚠ Washout overlap" requiring PI confirmation.
- **Metaphylaxis type column** (BR-2502 ConMed) — Metaphylaxis / Therapeutic / Preventive, with a confounding warning on metaphylactic (whole-pen, on-arrival) antibiotics — a known confounder in BRD efficacy trials.
- **Query density** (open queries per 100 CRF fields) — the standard data-quality metric, shown per site with rated thresholds (Excellent / Acceptable / Review / Action).
- **Protocol deviation tracking** — a per-site deviation count + a typed deviation table (Eligibility / Visit window / Dosing / Consent / Procedure, severity, reported-to-sponsor, status) per ICH E6 R2 §8.3.16.
- **SDV Completion Certificate** — a print-ready certificate (ICH E6 R2 + VICH GL9 attestation, fields/forms verified, CRA + DM signature lines, report ID) with a demo override that stamps a "DRAFT — SDV NOT COMPLETE" watermark.
- Print-styled (chrome hidden, table heads repeat) with CSV export per tabular report + a DM/Admin Export-all.

**VeDDRA coding architecture** — the platform mirrors how production EDCs (Medidata Rave, Oracle InForm) handle dictionary coding. Verbatim AE/ConMed terms are captured at the site; the VeDDRA field on the entry panels is **read-only for all roles** (the *result* of coding, not the input) with three states — **Coded** (green) / **Pending** (amber) / **Excluded** (slate, e.g. a saline flush that needs no coding). The DM codes terms **centrally in the Coding module** (the write path, next to build); the coded term then flows back to the originating record and lights up the panel field and the report column automatically. The display contracts are already wired.

**SDV module** (`/study/[studyId]/sdv`) — the source-data-verification worklist (one row per form instance with status / progress / open-query count) that **deep-links into the Subject Record with SDV mode active**, where the CRA verifies field-by-field (DM gets a read-only view) — plus the SDV Completion report + certificate stub above.

**AE / SAE tab** on the Subject Record — a subject-level safety view alongside the form tree: every adverse event for that subject with sequential AE-000N numbers, a summary stat row (Total / SAEs / Open / Resolved), severity / SAE / VeDDRA chips, and SAEs highlighted red — the per-subject lens on the same data the study-level AE Roster shows.

**Pending:**
- **Coding / VeDDRA module** — the DM's central dictionary-coding worklist (the write path that completes the VeDDRA architecture above)
- **Inventory module** — drug accountability + dispensing that resolves the Randomization form's lot/batch/kit links (Case Study 6's bridge wired end-to-end)
- **Case Study 7 — conditional demographics** (breed lists, production-purpose tags; age auto-calc is done)
