...
The screen is designed as a **triage queue, not a second editor**, and it makes the queries-vs-edit-checks distinction structural: **two tabs**. The **Queries** tab is the manual `Q-` worklist — query ID, subject, site, the **field** (label + code chip, with its group → form path as subtext), the **value that triggered it**, the query text, a status chip, a **days-open counter that turns red once it breaches the SLA** (>7 days raised, >3 days responded), and who the query is **assigned to** next (derived from status — a raised query is the site CRC's to answer, a responded one is the monitor's to resolve), so both the backlog and the next owner surface themselves. The triage columns (subject, site, status, days-open, assignee) are click-sortable, so a monitor can re-pivot the same queue by site one moment and by age the next — using the same **app-wide sort standard** (one shared hook + header component: click cycles ascending → descending → clear, one column at a time) that every list table in the product shares, so the interaction is identical whether you're in Queries, the Animals list, or the site drill-down. The **Edit Checks** tab is the open `EC-` anomalies, and it shows different columns because it answers a different question: the **value entered** beside its **normal range** ("40.1 °C" vs "38.3–39.2 °C"), so a monitor can scan for severity at a glance. The two click targets encode the two intents: clicking a **row opens the thread in place** (a 420px slide-in that reuses the record's own panel markup — you triage without losing the list), while clicking the **subject ID navigates into the record and deep-links straight to that field's panel** — for when triage isn't enough and you need the full clinical context around the value.

The same **role model** carries over verbatim — CRC responds, CRA resolves, DM manages, PI responds, **Sponsor and Admin are read-only**, and the Sponsor's view additionally **drops the entire Edit-Checks tab** because an unconverted edit-check can leak an out-of-range value the sponsor is blinded to. Edit-checks are visible to the DM (the full anomaly surface) but stay **non-actionable here** — they carry a "resolve on the subject record" note, because converting an edit-check is a clinical judgement that belongs next to the source, not in a queue. That same role lens drives the **sidenav badge**: it doesn't count *all* open queries — it counts the ones *this role can act on right now* (a CRC sees only what's awaiting their response; a CRA also sees what's awaiting resolution; a sponsor sees nothing), and it re-counts the moment the role switches or a query is resolved. The number on the nav rail is therefore a personal to-do count, not a study statistic. The worklist is where you *see and route* anomalies; the record is where you *resolve* them. One log, two views, each shaped to what its user is actually trying to do.
...
Not every check carries the same weight, and the engine encodes that gradient. Most are **advisory**: a CADESI-04 dermatitis score above 120 is unusual but not impossible, so it raises a *soft* edit-check — "verify with the investigator" — and the value is still accepted (a custom `message` on the field overrides the generic out-of-range text; a worksheet-upload field sits beside the score for the source document). A few are **absolute**: a beef animal still inside its drug-withdrawal period **cannot** be marked *Shipped* for slaughter — that's a food-safety **hard block** that stops form submission outright, with a red banner, and that the data-entry coordinator cannot override (only a Data Manager can, by recording a protocol deviation). Same validation layer, two postures — *advise* vs *forbid* — chosen per field by clinical and regulatory consequence. A generic EDC treats every rule as the same kind of nag; a domain-built one knows the difference between "double-check this" and "this animal cannot enter the food chain."

The field metadata keeps earning its keep beyond validation. A field can declare a **condition** (`showIf`) and appear only when it's relevant — the broiler study's investigational feed additive carries a GCP second-observer counter-signature that surfaces *only* for the phytogenic-treatment pens, required when shown but invisible (and non-blocking) for the controls — so the form reshapes itself to the arm without a separate form per arm. And every visit form opens with a read-only **protocol-version** stamp, resolved per study, so a value is always traceable to the protocol edition in force when it was recorded. Conditional display, regulated counter-signatures, version stamping — all of it is the same idea one more turn down: the form is data, and the data knows how it should behave.
...
The **audit trail is the proof the architecture already pays off**. Because every form carries its scope and every action resolves through the same instance → form → study chain, the 21 CFR Part 11 event log captures activity at *all* levels in one stream — a coordinator entering a pen-level vital, a monitor verifying a site-level regulatory record, a house-level environmental reading — each attributed to the right subject, barn, or site automatically. A patient-shaped schema would have nowhere to *hang* a barn-level or site-level audit event; here it falls out of the model for free. The multi-level data architecture isn't just cleaner forms — it's what makes a complete, level-aware compliance record possible in the first place. The same stream already carries the **regulatory milestones** that matter to an auditor — randomization, consent obtained, protocol deviations, and **emergency unblinding** (a double-blind reveal that writes a permanent, reason-stamped entry the moment a Data Manager breaks the blind) — every event UTC-timestamped, because a compliance log that drifts with the reader's time zone isn't one.

The **visit scheduler** is the same trick pointed forward instead of back. It stores no schedule of its own — a *visit* is derived as (subject × the visit forms the protocol already defines), the target date computed from the subject's enrolment baseline plus the day offset baked into each form, and a visit is "done" the moment its form reaches review. From that one derivation the screen knows what's **overdue**, **due this week**, or **completed outside its window** — across every active subject, ranked by urgency — with each study's own visit cadence and windows (the canine ±2-to-±5-day follow-ups, the feedlot's Day-0-to-28 schedule, the broiler's weekly weigh-ins) read straight from the form tree. Audit trail and visit scheduler are bookends on the same model: one reconstructs what *happened*, the other projects what's *owed* — and neither needed a table of its own to do it. And because the visit and the form are the same object, an overdue row's **Open** action drops you straight onto that exact visit form, highlighted in the record — the projection and the place you act on it are one click apart.
...
The demo data is held to the same clinical standard as the screens. A subject's **status and its data agree**: a *Completed* dog carries fully-populated visit forms with a realistic CADESI-04 / PVAS improvement curve across every visit and zero open queries (because a finalized-but-empty form, or a completed subject with an unanswered query, would be a GCP red flag that a reviewer in this field would spot in seconds); a *Withdrawn* dog has real data up to her withdrawal date and nothing after. The point of a portfolio demo isn't just that the screens render — it's that the data behind them is *coherent enough to be believed*.
