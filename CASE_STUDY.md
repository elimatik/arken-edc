...
The screen is designed as a **triage queue, not a second editor**, and it makes the queries-vs-edit-checks distinction structural: **two tabs**. The **Queries** tab is the manual `Q-` worklist — query ID (carrying a status-colour dot), subject, site, form, field, the **value that triggered it**, the query text, who **raised** it and who it's **assigned to** (derived from status — a raised query is the site CRC's to answer, a responded one is the monitor's to resolve), and a **days-open counter that turns red once it breaches the SLA** (>7 days raised, >3 days responded) so the backlog surfaces itself. The **Edit Checks** tab is the open `EC-` anomalies, and it shows different columns because it answers a different question: the **value entered** beside its **normal range** ("40.1 °C" vs "38.3–39.2 °C"), so a monitor can scan for severity at a glance. The two click targets encode the two intents: clicking a **row opens the thread in place** (a 420px slide-in that reuses the record's own panel markup — you triage without losing the list), while clicking the **subject ID navigates into the record and deep-links straight to that field's panel** — for when triage isn't enough and you need the full clinical context around the value.
