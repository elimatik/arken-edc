...
Two details make it a *real* EDC surface rather than a spreadsheet. First, **mirroring**: Submit writes each row to *that animal's own form instance* — the very record the Subject Record reads — so a temperature typed in the grid shows up, with its edit check, on the individual Vital Signs form. There is no second copy and no sync; the batch grid and the one-animal form are two views of one instance. Second, the **reason-for-change bar**: regulated data, once saved, can't be silently overwritten, so the moment you edit a value that was already on file the amber bar slides up asking *why* — one reason applied across every modified cell, or overridden per row — and each edit lands as a change-reason delta in the same audit flow the Subject Record uses. Queries, source-data verification, and the Submit→Finalize→Lock lifecycle deliberately *don't* appear on the grid: batch is for fast capture and correction; the regulated ceremony stays on the individual record where it belongs.
