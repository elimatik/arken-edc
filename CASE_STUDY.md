...
A `batch_eligible` flag on the form definition marks the recurring visit forms (BR-2502's Vital Signs and Clinical Response, all five visit days). Where any such form exists, a **Batch entry** button appears — on the Animals list and at the animal level of the drill-down — and *only* there (PH-2401 and CA-0801 have no batch-eligible forms, so they never show it). It opens a two-step flow: a **form picker** that reads each animal's enrollment date, applies the visit windows, and surfaces "Vital Signs — Day 3 · 5 animals due today" cards above the full list; then a **full-screen grid** with **no checkboxes** — every visible animal is in, because a pen is captured as a whole — one row per animal, one column per field, inline editing at design-system cell height, native selects, compact Yes/No toggles, a sticky animal-ID column. The same validation engine runs **per cell**: an out-of-range temperature flags the cell amber and drops an `EC- n` chip in the row's status, age-class-aware (the calf and the steer in the same pen get different heart-rate ranges) — but it never blocks the technician mid-pass, because the field reality is that you record first and reconcile after.

Two details make it a *real* EDC surface rather than a spreadsheet. First, **mirroring**: every cell writes straight to *that animal's own form instance* — the very record the Subject Record reads — so a temperature typed in the grid shows up, with its edit check, on the individual Vital Signs form. There is no second copy and no sync; the batch grid and the one-animal form are two views of one instance. That identity is what lets the grid carry the *whole* regulated flow inline instead of a stripped-down version of it: the same orange edit-check icon opens the same Edit Check panel (explain, or convert to a formal query); a Remarks "Queries" mode raises the same flag and the same query thread; and the moment you change a value that was already on file, the same dashed-red **Δ** appears on the cell and opens the same change-reason panel — old → new, a reason, the full history — landing a change-reason delta in the same audit trail. Second, the **boundary**: what the grid deliberately *doesn't* show is as considered as what it does. DM approval of a change, source-data verification, and the Submit → Finalize → Lock signature lifecycle never appear in batch — those are deliberate, one-record-at-a-time acts of accountability, and they stay on the individual Subject Record where a name attaches to them. Batch is for fast capture and correction at the chute; the regulated ceremony stays where it belongs. (A bottom summary bar still offers a one-reason shortcut across every pending change, for when a whole pass needs the same correction note — but the per-cell Δ is the real interaction.)
