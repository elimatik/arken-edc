...
The **audit trail is the proof the architecture already pays off**. Because every form carries its scope and every action resolves through the same instance → form → study chain, the 21 CFR Part 11 event log captures activity at *all* levels in one stream — a coordinator entering a pen-level vital, a monitor verifying a site-level regulatory record, a house-level environmental reading — each attributed to the right subject, barn, or site automatically. A patient-shaped schema would have nowhere to *hang* a barn-level or site-level audit event; here it falls out of the model for free. The multi-level data architecture isn't just cleaner forms — it's what makes a complete, level-aware compliance record possible in the first place.
